...
(async()=>{
 const off=await boot('');assert.equal(off.enabled,false);assert.equal(requests,0,'normal station does not fetch preview assets');assert.equal(off.accepts({t:'desk',x:2,y:2,w:2,h:1}),false,'disabled preview cannot request extra lighting samples');
 ...
 const p={id:'desk',t:'desk',x:2,y:2,w:2,h:1};assert.ok(r.accepts(p));assert.equal(r.accepts({...p,t:'plant'}),false,'unmodified equipment retains its existing lighting path');const idle=r.lightOf(p,false,null),view=r.lightOf(p,true,null),working=r.lightOf(p,true,{heat:.7});assert.equal(idle.a,view.a,'mere inspection is not fabricated work');assert.ok(working.a>idle.a);
