...
(async()=>{
 ...
 try {
  ...
  assert.equal(j.starnet.completed,true);assert.equal(j.choices[0].message.content,answer);assert.equal(j.usage.total_tokens,6);assert.equal(calls.length,2);assert.equal(calls[1].outputOnly,true);assert.equal(calls[1].reflect,false);
