...
/* ---- conveyor artwork follows the transport graph (clean remaster) ----
   ...
   direction/topology and route-state honesty; they do not replace the sim tests. */
{
  ...
  for (const dir of Object.keys(vec)) {
    const v = vec[dir], at = time => drawing(line(dir), time).marks.filter(r => target(r) && r.colour === '#627783')
  ...
  for (const from of Object.keys(vec)) for (const dir of Object.keys(vec)) {
    ...
    A.eq(d.marks.filter(r => target(r) && r.colour === '#92a3ae').length, 0, 'a real ' + from + ' to ' + dir + ' bend has no false feeder collar');
  }
  ...
  A.eq(falseFeeder.marks.filter(r => target(r) && r.colour === '#92a3ae').length, 1, 'a neighbour flowing elsewhere is not an incoming feeder');
  const isolated = drawing([B(0, 0, 'N')], 0);
  A.eq(isolated.marks.filter(r => r.colour === '#92a3ae').length, 1, 'a lone tile has its source collar');
  A.eq(isolated.marks.filter(r => r.colour === '#172530').length, 1, 'a lone tile also has its open-end chute');
