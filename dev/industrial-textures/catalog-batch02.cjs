...
async function main(){
 ...
 for(const [lane,ids]of Object.entries(assigned))for(const id of ids){
  const rel=base+'/'+lane+'/'+id+'.png',file=path.join(root,rel);if(!fs.existsSync(file))continue;
  ...
  items.push({id,lane,group:labels[lane],label:prop.label,image:'assets/industrial/batch02/'+lane+'/'+id+'.png',width:info.width,height:info.height,footprint:v.footprint,bounds:v.bounds,transparentPixels:transparent,opaquePixels:opaque,status:'exported-art-awaiting-live-review'});
 }
