...
const root='frontend/assets/industrial/catalog-utility-rest-4',docs='docs/station-remaster/catalog-utility-rest-4';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
const items=[
 ['treasury_coinsorter',40,125,610,360],['treasury_pnl_holo',750,270,365,210],['treasury_token_furnace',1235,30,240,465],
 ['war_pivotpanel',15,555,535,395],['war_threatcore',555,535,225,425],['wartable',0,0,0,0,'table-depth-source']
];
...
(async()=>{
 ...
 const layers=[],labels=[],cellW=640,cellH=225;
 for(let i=0;i<records.length;i++){
  ...
  for(const [file,fit,mult,x]of [[`frontend/assets/industrial/approved-sheet/${old.image}`,old.bounds,2,ox+10],[root+'/'+r.image,r.actualFit,2,ox+180],[root+'/'+r.image,r.actualFit,4,ox+350]]){
 ...
 const label=Buffer.from(`<svg width="1280" height="675"><style>text{font:14px sans-serif}</style>${labels.join('')}</svg>`);
 layers.push({input:label,left:0,top:0});
 await sharp({create:{width:1280,height:675,channels:4,background:'#333b3c'}}).composite(layers).png().toFile(docs+'/game-scale-proof.png');
