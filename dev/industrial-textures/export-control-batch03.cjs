...
const root=path.resolve(__dirname,'../..'),dir='frontend/assets/industrial/batch03/control/';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
async function main(){
 const list=JSON.parse(fs.readFileSync(path.join(root,'docs/station-remaster/batch03/control/sources.json')));
