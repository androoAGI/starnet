...
const root=path.resolve(__dirname,'../..'),dir='frontend/assets/industrial/batch03/communications/';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
async function main(){
 const list=JSON.parse(fs.readFileSync(path.join(root,'docs/station-remaster/batch03/communications/sources.json')));
 ...
 for(const a of list){
  ...
  if(!hasAlpha){
   const visit=i=>{if(i<0||i>=n||seen[i])return;const p=i*4,lo=Math.min(data[p],data[p+1],data[p+2]),hi=Math.max(data[p],data[p+1],data[p+2]);if(lo<(a.neutralMin||145)||hi-lo>25)return;seen[i]=1;queue[end++]=i;};
 ...
 fs.writeFileSync(path.join(root,dir,'export-checks.json'),JSON.stringify({version:1,method:'User-authorized connected neutral-background extraction; original RGB preserved',records},null,2)+'\n');
}
