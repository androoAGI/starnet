...
const docs='docs/station-remaster/catalog-crew-rest-4',out='frontend/assets/industrial/catalog-crew-rest-4';
...
(async()=>{
 ...
 for(const cell of layout.cells){
  ...
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
   const src=((y+top)*info.width+x+left)*4,d=(y*w+x)*4;data.copy(raw,d,src,src+4);if(cell.separation){const sep=cell.separation,cut=y+top<sep.breakY||y+top>=sep.endY?sep.upperX:sep.lowerX;if(sep.side==='left'?x+left>=cut:x+left<cut)raw[d+3]=0;}maxAlpha=Math.max(maxAlpha,raw[d+3]);
  ...
  records.push({id:cell.id,view:'s',image,sourceWidth:crop.width,sourceHeight:crop.height,bounds,footprint:cell.footprint,surfaceSupport:cell.surfaceSupport,
   ...
   sourceSeparation:cell.separation,source:sourcePath,sourceSha256:hash(sourceBytes),sourceCrop:{left:left+l,top:top+t,width:crop.width,height:crop.height},outputSha256:hash(png),
