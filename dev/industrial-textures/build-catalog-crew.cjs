...
const docs='docs/station-remaster/catalog-crew',out='frontend/assets/industrial/catalog-crew';
...
(async()=>{
 ...
 const receipt={version:1,count:records.length,scope:'Eight primary s views only. Existing alternate facings are not replaced or claimed as corrected.',method:'Generated RGBA source preserved. Alpha>=180 core with radius3 preserves nearby original edge RGBA; remove detached fog/alpha1. Tight crop only, no RGB or retained alpha modification.',refs:layout.refs,records};
 ...
 for(let i=0;i<records.length;i++){
  ...
  composites.push({input:await sharp(a.image).resize(w,h).png().toBuffer(),left:20+(i%4)*190+Math.floor((160-w)/2),top:35+Math.floor(i/4)*170});
  composites.push({input:Buffer.from('<svg width="185" height="22"><text x="0" y="16" fill="#dddcca" font-size="12">'+a.id+'</text></svg>'),left:10+(i%4)*190,top:5+Math.floor(i/4)*170});
 }
 await sharp({create:{width:780,height:340,channels:4,background:'#343c3c'}}).composite(composites).png().toFile(docs+'/fit-qa.png');
