...
const root='frontend/assets/industrial/catalog-machinery',docs='docs/station-remaster/catalog-machinery';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
const items=[
 ['tank',0,30,391,380,'proportions-source'],['tube',380,160,360,250],
 ['fabricator',695,90,435,335,'proportions-source'],['vat',1135,90,401,340,'proportions-source'],
 ['industrial_servicecab',95,465,215,490,'proportions-source'],['cryopod',440,450,240,510],
 ['incubator',810,445,225,520,'proportions-source'],['comms_uplink',1160,425,310,540]
];
...
(async()=>{
 ...
 for(const [id,left,top,w,h,selectedSource='sheet-source']of items){
  const source=docs+'/'+selectedSource+'.png',bytes=fs.readFileSync(source),decoded=await sharp(bytes).ensureAlpha().raw().toBuffer({resolveWithObject:true});
  ...
  if(width*scale<bounds.width*.9-1e-6||height*scale>bounds.height+1e-6)throw Error('Source proportion outside 10% width/maximum-height gate: '+id+' '+(width*scale)+'x'+(height*scale));
  records.push({id,view:'s',image,repoPath:root+'/'+image,sourceWidth:width,sourceHeight:height,
   ...
   verification:{transparent,retained:nonzero,removedAlpha,retainedRGBAMismatches:rgbaMismatches,allRGBMismatches:rgbMismatches,widthFraction:f(width*scale/bounds.width),heightWithinEnvelope:height*scale<=bounds.height+1e-6},
