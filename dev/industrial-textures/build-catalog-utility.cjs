...
const root='frontend/assets/industrial/catalog-utility',docs='docs/station-remaster/catalog-utility';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
const items=[
 ['coffee',70,130,300,370],['quarters_minifridge',430,120,270,380],
 ['quarters_vending',770,0,350,500],['jukebox',1140,60,370,440],
 ['pixelrig',15,560,445,425],['intake',470,505,465,480],
 ['core',945,495,300,490],['rackV',1250,490,260,495]
];
const f=n=>+n.toFixed(6);
(async()=>{
 const structure=JSON.parse(fs.readFileSync('dev/industrial-textures/prop-structure-manifest.json')).props;
 const source=docs+'/sheet-source.png',bytes=fs.readFileSync(source),decoded=await sharp(bytes).ensureAlpha().raw().toBuffer({resolveWithObject:true});
 const records=[];fs.mkdirSync(root,{recursive:true});
 for(const [id,left,top,w,h]of items){
  ...
  records.push({id,view:'s',image,repoPath:root+'/'+image,sourceWidth:width,sourceHeight:height,
   ...
   verification:{transparent,retained:nonzero,removedAlpha,retainedRGBAMismatches:rgbaMismatches,allRGBMismatches:rgbMismatches},
