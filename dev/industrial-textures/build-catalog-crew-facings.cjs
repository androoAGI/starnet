...
const docs='docs/station-remaster/catalog-crew-facings',out='frontend/assets/industrial/catalog-crew-facings';
...
(async()=>{
 ...
 for(const cell of layout.cells){
  ...
  const crop={left:l,top:t,width:r-l+1,height:b-t+1},image=out+'/'+cell.id+'-'+cell.view+'.png';
  ...
  records.push({id:cell.id,view:cell.view,image,sourceWidth:crop.width,sourceHeight:crop.height,bounds,footprint:cell.footprint,
 ...
 const receipt={version:1,count:records.length,scope:'Nine supported alternate facings; four associated primary designs unchanged.',method:'Generated RGBA source preserved. Alpha>=180 core with radius3 preserves nearby original edge RGBA; remove detached fog/alpha1. Tight crop only, no RGB or retained alpha modification.',refs:layout.refs,records};
 fs.writeFileSync(docs+'/exports.json',JSON.stringify(receipt,null,2)+'\n');
 fs.writeFileSync(docs+'/integration.json',JSON.stringify({version:1,status:'candidate-not-integrated',records:records.map(({id,view,image,sourceWidth,sourceHeight,bounds,footprint,contact,otherSupportedViewsUnchanged})=>({id,view,image,sourceWidth,sourceHeight,bounds,footprint,contact,otherSupportedViewsUnchanged}))},null,2)+'\n');
 const composites=[];
 for(let i=0;i<records.length;i++){
  const a=records[i],w=Math.max(1,Math.round(a.fit.width*4)),h=Math.max(1,Math.round(a.fit.height*4));
  composites.push({input:await sharp(a.image).resize(w,h).png().toBuffer(),left:20+(i%3)*190+Math.floor((160-w)/2),top:35+Math.floor(i/3)*170});
  composites.push({input:Buffer.from('<svg width="185" height="22"><text x="0" y="16" fill="#dddcca" font-size="12">'+a.id+':'+a.view+'</text></svg>'),left:10+(i%3)*190,top:5+Math.floor(i/3)*170});
 }
 await sharp({create:{width:590,height:550,channels:4,background:'#343c3c'}}).composite(composites).png().toFile(docs+'/fit-qa.png');
