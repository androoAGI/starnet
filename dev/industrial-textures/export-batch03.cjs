...
const root=path.resolve(__dirname,'../..'),dir='frontend/assets/industrial/batch03/coordinator/';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
async function main(){
 const list=JSON.parse(fs.readFileSync(path.join(root,'docs/station-remaster/batch03/coordinator/sources.json')));
