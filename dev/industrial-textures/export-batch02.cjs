...
const root=path.resolve(__dirname,'../..'),dir='frontend/assets/industrial/batch02/coordinator/';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
async function main(){
 const list=JSON.parse(fs.readFileSync(path.join(root,'docs/station-remaster/batch02/coordinator/sources.json')));
