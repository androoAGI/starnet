...
async function prepare(source,id){
 ...
 const out=path.join(root,'frontend/assets/industrial/props-v2');fs.mkdirSync(out,{recursive:true});
 ...
 const meta={id,source,sourceWidth:w,sourceHeight:h,crop,sourceAlpha:hasAlpha,subjectRgbChanges:0,output:'frontend/assets/industrial/props-v2/'+id+'.png'};
 const docs=path.join(root,'docs/station-remaster/props-v2');fs.mkdirSync(docs,{recursive:true});fs.writeFileSync(path.join(docs,id+'.export.json'),JSON.stringify(meta,null,2)+'\n');console.log(JSON.stringify(meta));
}
const command=process.argv[2];(command==='guides'?guides():command==='prepare'?prepare(process.argv[3],process.argv[4]):Promise.reject(Error('Use guides or prepare <source> <id>'))).catch(e=>{console.error(e);process.exitCode=1;});
