...
(async () => {
  const provider = await startOverseerProvider({ workerDelay: 18000 });
  const profile = path.resolve('.dogfood/project/profile'); fs.mkdirSync(profile, { recursive: true });
  const port = Number(process.env.STARNET_PORT) || await allocatePort();
  const child = spawn(process.execPath, ['dev/seed.js', '--keep', '--workspace', path.resolve('.dogfood/project/workspace')], { windowsHide: true, stdio: 'inherit', env: {
