 ...
 function mount({host,bridge,agentId,onError}){
  ...
  async function load(kind){const current=++revision;body.replaceChildren(make('p','Loading recorded work…','muted'));try{
   if(kind==='sessions'){
    ...
    for(const s of items){const b=make('button',s.title||'General conversation','run-card record-link');b.onclick=async()=>{try{const data=await bridge.readTranscript(s.id,{agentId:s.agentId||agentId,limit:200});if(current!==revision)return;body.replaceChildren();const back=make('button','← Conversations','action');back.onclick=()=>load('sessions');body.append(back,make('p',s.title||'General conversation','section-title'));for(const t of data.turns||[]){const row=make('div',null,'message '+(t.role==='user'?'user':''));row.append(make('span',t.role==='user'?'YOU':t.agentId||t.role,'speaker'),make('span',typeof t.content==='string'?t.content:JSON.stringify(t.content)));body.append(row);}if(!data.turns?.length)body.append(make('p','No durable transcript turns are available for this session.','muted'));}catch(e){onError(e.message);}};body.append(b);}
   }else if(kind==='files'){
