...
(() => {
  ...
  const height=20; // User-approved standing height for the industrial skin rollout.
  DATA.SKINS.readability_secretagent={name:'Secret Agent readability study',set:'readability_secretagent',scale:20/76};
  for(const [skin,id,name]of skins) {
    ...
    DATA.SKINS['readability_'+id]={name:name+' readability study',set:'readability_'+id,scale:20/76};
  }
  ...
  panel.innerHTML='<strong>INDUSTRIAL CREW · LIVE STATION DEMO</strong><div style="margin:7px 0">Normal station movement · no agent tasks running</div><label>Sprite height <input aria-label="Sprite height" type="range" min="16" max="30" value="20" style="width:110px;vertical-align:middle"> <output>20 px</output></label><div style="margin-top:7px"><label>Follow <select aria-label="Follow agent" style="background:#1a2528;color:#d8ceae;padding:3px"><option value="">Station view</option></select></label> <a href="agent-motion-review.html" style="color:#98c5ca">Earlier frame study</a></div><div id="agent-demo-status" style="margin-top:7px">Loading station…</div>';
  ...
  const showReview=z=>{panel.querySelector('input').value='20';panel.querySelector('output').textContent='20 px';for(const[skin]of skins)DATA.SKINS[skin].scale=20/76;World.showSkinReview(true,z,skinPicker.value);};
  ...
  const timer=setInterval(async()=>{
    ...
    try{
      ...
      panel.querySelector('#agent-demo-status').textContent='20 px baseline · five animated skins · five new front studies';
      panel.dataset.revision='industrial-rollout-20px';
