/* STARNET — conveyor.js : directional belts + the boxes that ride them.
   ...
   Clean Cadet materials: off-white machined rails, graphite roller beds, restrained cyan
   drive markers and clean white cargo modules. Small payload tags retain the semantic
...
const Conveyor = (() => {
  ...
  /* ============================ CARGO ART ============================ */
  // All modules use the same handled white enclosure and 9x8 floor stance. The
  // small face insert carries role colour; broad clean panels carry the material.
  function cargoChassis(cx, py, h32, body, dir) {
    const x = cx - 4, y = py - 5;
    px(x, y, 9, 8, '#35424e');
    px(x + .5, y + .5, 8, 4.5, '#dce4e8');
    px(x + 1, y + 1, 7, .5, '#f5f8f9');
    px(x + .5, y + 5, 8, 2.5, '#8595a2');
    px(x + 1, y + 5, 7, .5, '#b5c2cb');
    px(x + 1.5, y + 2, 6, 2, body);
    px(x + 2, y + 2, 5, .5, SH(body, .2));
    // Recessed grab handles and short corner guards share the prop panel grammar.
    px(x + 1, y + 1.5, .5, 2.5, '#6d7e8b');
    px(x + 7.5, y + 1.5, .5, 2.5, '#6d7e8b');
    px(x + 3, y + 6, 3, .5, '#4f6272');
    const v = DIRV[dir] || DIRV.E;
    if (v[0]) px(cx + v[0] * 3.5, y + 2, .5, 2, '#f5f8f9');
    else px(x + 3, v[1] < 0 ? y + .5 : y + 4.5, 3, .5, '#f5f8f9');
    return { tx: x + 1, ty: y };
  }

  function cargoProduction(cx, py, h32, dir) {
    cargoChassis(cx, py, h32, '#7d929c', dir);
    px(cx - 1.5, py - 2.5, 3, .5, '#d4e6ed');
  }
  function cargoUtility(cx, py, h32, dir) {
    cargoChassis(cx, py, h32, '#a5b3bd', dir);
    px(cx - 1, py - 3, 2, 1, '#dce6eb');
  }
  function cargoData(cx, py, h32, dir) {
    cargoChassis(cx, py, h32, '#476573', dir);
    px(cx - 1.5, py - 2.5, 3, .5, '#a9dce7');
    px(cx + 1, py - 3, .5, 1, '#d2f0f4');
  }
  function cargoCommand(cx, py, h32, dir) {
    cargoChassis(cx, py, h32, '#826b70', dir);
    px(cx, py - 3, .5, 1.5, '#f0b5b0');
  }
  function cargoMoney(cx, py, h32, dir) {
    cargoChassis(cx, py, h32, '#7f887a', dir);
    px(cx - 1.5, py - 3, 1, 1, '#d4dec8');
    px(cx + .5, py - 3, 1, 1, '#d4dec8');
  }
  // Incoming work has a uniform module size: it cannot imply a cost estimate.
  function cargoOre(cx, py, h32, dir, weight) {
    cargoChassis(cx, py, h32, '#938978', dir);
    px(cx - 1.5, py - 2.5, 3, .5, '#ded4ba');
  }
  ...
  function cargoProduct(cx, py, h32, dir, weight) {
    ...
    cargoChassis(cx, py, h32, '#657e78', dir);
    px(cx - .5, py - 3, 1, 1, '#c4e8d9');
    for (let i = 0; i < Math.round(w * 2); i++) px(cx - 2 + i * 4, py - 2.5, .5, .5, '#d9f0e5');
  }
  function cargoSlag(cx, py, h32, dir) {
    cargoChassis(cx, py, h32, '#80646b', dir);
    px(cx - 1, py - 3, .5, .5, '#edb2ad'); px(cx + .5, py - 3, .5, .5, '#edb2ad');
    px(cx - .5, py - 2.5, 1, .5, '#edb2ad');
  }
  ...
  function payloadTag(cx, py, role) {
    ...
    px(cx - .25, yy + 2.5, .5, 1.5, '#647481');
    px(cx - 2.5, yy, 5, 3, '#35424e');
  ...
  function create(opts) {
    ...
    function drawMergeFx(T) {
      ...
      for (const fx of mergeFx) {
        ...
        _ctx.fillStyle = '#bedfe8';
        _ctx.fillRect(cx - 3, cy - 3, 6, 6);                        // hot core
        _ctx.globalAlpha = 0.8 * k;
        _ctx.strokeStyle = '#bedfe8'; _ctx.lineWidth = 1;
    ...
    const BELT = { outline: '#35424e', rail: '#c6d1d9', bed: '#364650', tread: '#627783',
      treadLight: '#8596a0', groove: '#24333e', mark: '#b4c6d0', light: '#dce7ec' };
    ...
    function beltPathStroke(paths, width, colour) {
      _ctx.strokeStyle = colour; _ctx.lineWidth = width; _ctx.lineJoin = 'round'; _ctx.lineCap = 'butt';
      _ctx.beginPath();
    ...
    function beltTile(X, Y, T, info, now, live) {
      ...
      beltPathStroke(paths, T - 1, BELT.rail);
      beltPathStroke(paths, T - 3.5, BELT.groove);
      beltPathStroke(paths, T - 4.5, BELT.bed);
      ...
      for (const p of paths) for (let d = scroll; d < T; d += pitch) {
        ...
        if (v[0]) {
          px(x - .3, y - (T - 5.5) / 2, .6, T - 5.5, BELT.tread);
          px(x - .3, y - (T - 5.5) / 2, .2, T - 5.5, BELT.treadLight);
        } else {
          px(x - (T - 5.5) / 2, y - .3, T - 5.5, .6, BELT.tread);
          px(x - (T - 5.5) / 2, y - .3, T - 5.5, .2, BELT.treadLight);
        }
      }
      ...
      // Reduce contrast without painting the floor outside the rounded track.
      if (!live) { _ctx.globalAlpha = .22; beltPathStroke(paths, T - 1, '#24333e'); }
      _ctx.restore();
    }
    ...
    function beltSource(X, Y, T, info) {
      ...
      if (v[0]) { px(cx - .5, Y + 2, 1, T - 4, '#92a3ae'); px(cx - .5, Y + 2, .3, T - 4, BELT.light); }
      else { px(X + 2, cy - .5, T - 4, 1, '#92a3ae'); px(X + 2, cy - .5, T - 4, .3, BELT.light); }
    }
    function beltSink(X, Y, T, info) {
      const v = DIRV[info.dir], cx = X + T / 2 + v[0] * (T / 2 - 1), cy = Y + T / 2 + v[1] * (T / 2 - 1);
      if (v[0]) { px(cx - .6, Y + 2, 1.2, T - 4, '#172530'); px(cx - .6, Y + 2, .3, T - 4, '#82949f'); }
      else { px(X + 2, cy - .6, T - 4, 1.2, '#172530'); px(X + 2, cy - .6, T - 4, .3, '#82949f'); }
    }
