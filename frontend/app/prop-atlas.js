...
(async()=>{
 ...
 function frame(now){
  ...
  for(const c of cards){if(!c.visible||!c.inView)continue;const {g,p,cv}=c;g.setTransform(1,0,0,1,0,0);g.fillStyle='#10191a';g.fillRect(0,0,cv.width,cv.height);
   ...
   PropSprites.draw({t:'crate',x:14,y:ground-1,w:2,h:1},false);g.imageSmoothingEnabled=true;g.drawImage(body,204-body.width*.385/2,ground*12-foot*.385,body.width*.385,body.height*.385);g.restore();
