...
const WorldRenderer = (() => {
  ...
  function create(options) {
      ...
      if (frame.geo !== geometry || frame.cache !== baked) {
        ...
        if (canLight()) {
          ...
          lighting.setGeometry(geometry, { width: baked.W, height: baked.H,
            tileSize: geometry.TILE, interiorPath: baked.interiorPath, surfaceMask: baked.baseCv,
    ...
    function drawLight(ctx, lights, params) {
      ...
      const t = clock();
      const rendered = lighting.render(ctx, Object.assign({ lights: lights || [], fixtures: baked.flickers || [],
    ...
    return { begin, drawBase, drawEntities, drawGrounding, drawLight, finish, stats, dispose };
  }
  return { GENERATION, enabled: () => !classic, create, visibleRect, intersects, sortedItems, percentile };
})();
