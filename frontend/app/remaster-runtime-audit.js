...
(async()=>{
  ...
  const css=document.createElement('style');css.textContent='#kepler-review,#skin-study-preview{display:none!important}#runtime-audit{position:fixed;z-index:99999;left:8px;top:50px;max-width:calc(100vw - 36px);background:#091214ed;color:#d7c89d;border:1px solid #746546;padding:8px;font:12px monospace}#runtime-audit button,#runtime-audit select{background:#172124;color:#d7c89d;border:1px solid #61716b;padding:4px;margin:2px;font:12px monospace}#runtime-audit output{display:block;white-space:pre-wrap;max-width:700px}#runtime-audit pre{max-height:200px;overflow:auto;font-size:10px}';document.head.append(css);
  ...
  button('Run catalog checks',async()=>{
    ...
    for(page=0;page<pageSelect.options.length;page++){
        ...
        for(const v of entries()){
          ...
          if(walkAll.checked){
            ...
            if(plan.route){
              ...
              walk={arrived:distance<1.1,distance:+distance.toFixed(2),ms:Math.round(performance.now()-started),samples,maxStep:+maxStep.toFixed(2)};
