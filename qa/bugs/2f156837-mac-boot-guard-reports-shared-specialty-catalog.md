...
September 18: REOPENED. New diagnostics explicitly show all five retries exhausted; the retry hardening did not resolve this report. Source repair under verification embeds the authoritative catalog in the staged desktop frontend and uses a same-origin static script tag. The engine startup/loopback request is removed from this boot-data path. Exhausted retries now report script load failure, not an unproven engine outage. The original network failure and installed recovery remain unknown.
