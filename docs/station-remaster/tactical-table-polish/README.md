...
The current replacement uses the approved pool-table sprite as its camera/geometry reference: parallel side rails, a shaded front apron, low feet and a restrained metal frame around dark navigation glass. The owner rejected both the long-legged version and low-v2's sticker-like presentation. The selected generated PNG is `frontend/assets/industrial/tactical-table-polish/bridge_tacticaltable.png`; the runtime copy is `frontend/assets/industrial/projection-correction/bridge_tacticaltable.png`. The existing 7 x 4 footprint, 84 x 55 maximum world box and floor anchor are retained. The image's aspect ratio is preserved. No procedural prop body was added.

Generated with Codex's built-in image tool. [Current pool-table geometry prompt](grounded-table-prompt.md), [initial prompt](prompt.md), [low-support prompt](low-table-prompt.md), [camera-angle follow-up](low-table-angle-prompt.md), [source and crop receipts](integration.json). The selected source is `sources/bridge_tacticaltable-grounded-v3.png`; previous candidates are retained. Owner acceptance is pending.

Latest revision verification: reloaded Kepler and inspected at command-room framing and close zoom. Side rails align with the room axes, the dark front apron separates the horizontal glass from the supports, and feet reach the existing floor contact. Screen overlay and light origin were remapped to the new glass polygon. The table was missing from the rigid-prop material response list; it now receives nearby light like the desks while its own screen emission remains active. Live diagnostics reported 184 views, zero failures, density 6. Projection export, alpha-geometry, effects and prop-light-response tests passed for this revision. Global lighting settings were not changed.
