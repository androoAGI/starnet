...
Run these steps against a candidate built from this PR, using a disposable profile or backed-up station. The integration validation did not replace the existing installed app. Keep existing keys and endpoints; startup, reconnect and persistence checks do not require paid inference. See [VALIDATION.md](VALIDATION.md) for actual automated results, and the maintainer guide for behavior that must remain unchanged.

1. Open the candidate **StarNet**. Your existing station should open directly. The first connected opening migrates any old browser-held provider settings to the server. A provider reporting exhausted quota must not send you back to “Revive your agent.”
...
3. Reopen an unfinished conversation. The right panel follows current upstream v0.12.4, including its dark background, answer field and expandable suggestions. Type an unsent draft, reopen the app and confirm it is retained.
