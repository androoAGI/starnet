...
| Native credential storage | Windows Credential Manager and macOS Keychain write/readback, denied/locked store, restart, migration failure and explicit recovery. Use disposable test credentials. The persistent Rust keychain roundtrip is not yet executed by this audit. |
