...
const result={cases,skins:catalog.length,maxMovingFacingError:maxError,artificialStrideDistance:0,speechFootDrift:Math.max(...feet)-Math.min(...feet),conversationOverlap:false};fs.writeFileSync('output/agent-animation-study/motion-polish-19px/regression.json',JSON.stringify(result,null,2)+'\n');console.log(result);console.log('Motion regression: PASS');
