...
const root='C:/Users/andro/gen-trees/sprite-reimagine-0914',base='output/agent-animation-study/rollout-20px',prepared=new Set();
...
const initial=await run(`node -e "const fs=require('fs'),b='${base}';console.log(JSON.stringify({queue:JSON.parse(fs.readFileSync(b+'/queue.json')),jobs:fs.readdirSync(b+'/jobs').filter(f=>f.endsWith('.json')).map(f=>JSON.parse(fs.readFileSync(b+'/jobs/'+f)))}));"`,100000);
...
while(queue.some(j=>!j.packed&&!j.failed)){
 ...
 for(const j of queue){
  if(active.length>=8)break;if(j.packed||j.failed||j.job||!ready.has(j.id))continue;
  ...
  if(!prepared.has(j.id)){const p=await run(`node output/agent-animation-study/prepare-catalog-refs.cjs ${j.id}`);if(p.exit_code!==0){j.error=p.output;j.failed=true;await save(jobFile(j),j);notify({id:j.id,preparationError:p.output});continue;}prepared.add(j.id);}
 ...
 for(const j of queue.filter(j=>j.done&&!j.packed&&!j.failed)){
  const p=await run(`node output/agent-animation-study/pack-catalog-minimax.cjs ${jobFile(j)}`);if(p.exit_code!==0){j.failed=true;j.error=p.output;await save(jobFile(j),j);notify({id:j.id,track:j.track,dir:j.dir,packingError:p.output});continue;}
