...
const root='C:/Users/andro/gen-trees/sprite-reimagine-0914',base='output/agent-animation-study/pixel-cadets-0915',prepared=new Set();
...
while(true){
 ...
 for(const j of queue){
  ...
  if(!prepared.has(j.id)){const p=await run(`node output/agent-animation-study/pixel-cadets-0915/prepare-refs.cjs ${j.id}`);if(p.exit_code!==0){j.error=p.output;j.failed=true;await save(jobFile(j),j);notify({id:j.id,preparationError:p.output});continue;}prepared.add(j.id);}
 ...
 for(const j of queue.filter(j=>j.done&&!j.packed&&!j.failed)){
  const p=await run(`node output/agent-animation-study/pixel-cadets-0915/pack-motion.cjs ${jobFile(j)}`);if(p.exit_code!==0){j.failed=true;j.error=p.output;await save(jobFile(j),j);notify({id:j.id,track:j.track,dir:j.dir,packingError:p.output});continue;}
